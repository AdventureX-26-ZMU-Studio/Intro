...
# --- interactive input ---
read -p "CNB repo slug  (e.g. buzhi/ZMU/Introduction): " -i "${CNB_EXTRACTED_SLUG:-}" CNB_SLUG
...
# --- configure git dual push ---
echo ""
echo "--- configuring git dual-push remotes ---"

# ensure origin exists
if ! git remote get-url origin >/dev/null 2>&1; then
  echo "[!] No 'origin' remote found. Please set it up first."
else
...
if command -v cnb &>/dev/null; then
  if cnb status 2>&1 | grep -q '已登录'; then
    echo "[OK] cnb logged in"
  else
    echo "[!] cnb NOT logged in — run: cnb login"
  fi
else
...
echo ""
echo "  3. Each team member runs:"
echo "     cnb login && gh auth login"
echo "     git config user.name \"Your Name\""
echo "     git config user.email \"you@example.com\""
echo "     ./scripts/setup-sync.sh"
echo ""
echo "  4. Commit & push .sync-config so pipelines work:"
echo "     git add .sync-config && git commit -m 'configure sync targets'"
